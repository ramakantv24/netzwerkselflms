<?php
    ...
    if(isset($_POST['course_id'])){
		...
		//Update user
		$sql2 = "UPDATE `users` SET `user_status`='".$_POST['user_status']."',`user_id`='".$userid."' WHERE `ID`='".$user_id."'";
		...
		if(($userRow['subscription_time'] == '') && ($userRow['start_date'] == '') && ($userRow['expire_date'] == '') && ($userRow['user_status'] == 'C')){
			...
			$sql23 = "UPDATE `users` SET `subscription_time`='".$_POST['subscription_time']."',`start_date`='".$start_date."',`expire_date`='".$expire_date."',`user_id`='".$userid."' WHERE `ID`='".$user_id."'";
