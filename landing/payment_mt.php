...
function sendThankYouEmail($emaill, $fname) {
    ...
    $mail->addReplyTo('info@netzwerkacademy.com', 'Netzwerk Support Team');
    $mail->isHTML(true);                                  // Set email format to HTML
    $mail->Subject = 'Congrats! You have enrolled for Manual Testing';
    $mail->Body = ' <html>
        <head>
        <title>Account Creation</title>
        </head>
        <body>
        <p>Hello '.$fname.',</p>
        <p>Thanks for joining our Manual Testing course. We are excited to have you onboard.</p>
...
if (isset($postdata ['key'])) {
	...
	//echo $productInfo;
    if($amount=="499" AND $status=='success' AND $productInfo=="Manual Testing"){
