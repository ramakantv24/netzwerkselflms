<?php
	...
	$sql22 = "SELECT * FROM `users` WHERE `user_status`='D' AND `update_date` < NOW() - INTERVAL 30 DAY";
	if($result2 = mysqli_query($con, $sql22)){
	    while ($row2 = mysqli_fetch_row($result2)) {
	        $user_id = $row2[0];
	       $courseUserSQL = "DELETE FROM `users` WHERE ID='".$user_id."'";
                        ...
                        <tbody>
                            <?php
							    if(!empty($_GET['search'])) {
									$sql = "SELECT * FROM `users` where email='".$_GET['search']."' AND user_status = 'D' ORDER BY `users`.`ID` DESC";
								}else{									
                                   $sql = "SELECT * FROM `users` where user_status = 'D' ORDER BY `users`.`ID` DESC";
								}
                                        ...
                                        echo '<tr>
                                              ...
                                              <td>
											    <a href="editUserCourse.php?id='.$u_id.'"><button type = "button" class = "btn btn-info">Edit</button></a>
											     <a href="deleting_users.php?delete_id='.$u_id.'" onclick="return deleteValidate();">
